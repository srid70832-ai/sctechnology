      ...
      <main className="flex-1 py-12 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full space-y-8">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-b from-[#0F172A] to-[#0B0F19] border border-blue-500/30 shadow-2xl space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-800 text-xs text-slate-300">
            <div>
              ...
              <span className="font-bold text-white">{formatISTDate(hackathon.startDate)}</span>
            </div>
            <div>
              <span className="text-slate-500 block text-[10px]">End Date / Submission</span>
              <span className="font-bold text-white">{formatISTDate(hackathon.endDate)}</span>
            </div>
            <div>
              <span className="text-slate-500 block text-[10px]">Registration Deadline</span>
              <span className="font-bold text-amber-400">{formatISTDate(hackathon.registrationDeadline)}</span>
            </div>
            <div>
              <span className="text-slate-500 block text-[10px]">Participants</span>
              <span className="font-bold text-white">{hackathon.participantsCount || 0} Registered</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 border-b border-slate-800 pb-3 overflow-x-auto">
          {[
            ...
            ...(hackathon.isRegistered ? [{ id: "team", label: hackathon.userTeam ? "My Team Hub" : "Registration Status" }] : []),
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-3">
              ...
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{hackathon.description}</p>
            </div>

            {hackathon.eligibility && (
              <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider">Eligibility</h3>
                <p className="text-xs text-slate-300">{hackathon.eligibility}</p>
              </div>
            )}
          </div>
        )}

        {/* Tab Contents: Problem Statements */}
        {activeTab === "problem" && (
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-blue-400">
                <Sparkles className="w-5 h-5" />
                <h3 className="text-sm font-bold uppercase tracking-wider">Official Problem Statements</h3>
              </div>
              <span className="text-xs text-slate-400 font-medium">
                {hackathon.problemStatements?.length ? `${hackathon.problemStatements.length} Challenges` : "Official Track"}
              </span>
            </div>

            {hackathon.problemStatements && hackathon.problemStatements.length > 0 ? (
              <div className="space-y-4">
                {hackathon.problemStatements.map((prob: any, idx: number) => (
                  <div key={prob.id || idx} className="p-5 rounded-2xl bg-slate-950 border border-slate-800/80 space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed">{prob.fullProblemDescription || prob.shortDescription || prob.description}</p>
                    {prob.technologySuggestions && prob.technologySuggestions.length > 0 && (
                      <div className="flex flex-wrap gap-1 pt-1">
                        <span className="text-[10px] text-slate-500 self-center mr-1">Suggested Stack:</span>
                        {prob.technologySuggestions.map((t: string, i: number) => (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[10px] font-bold border border-indigo-500/30">
                        {hackathon.userTeam.roundStatus.replace(/_/g, " ")}
                      </span>
                    </div>
                  ...
                  <div className="px-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-right">
                    ...
                    <span className="text-sm font-bold text-white">
                      {hackathon.userTeam.members?.length || 1} / {hackathon.userTeam.maxTeamSize || 4} Members
                    </span>
                  </div>
                </div>
                    ...
                    <table className="w-full text-left text-xs text-slate-300">
                      <thead className="bg-slate-950 text-slate-500 font-bold border-b border-slate-800">
                        <tr>
                          ...
                          {hackathon.isLeader && <th className="py-3 px-4 text-right">Action</th>}
                        </tr>
                      </thead>
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-6">
            {!hackathon.isRegistered ? (
              ...
              <div className="p-8 text-center rounded-2xl bg-slate-950 border border-slate-800 space-y-4">
                ...
                {hackathon.userTeam.submission && (
                  <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 max-w-md mx-auto text-left space-y-2 text-xs">
                    <span className="text-[10px] text-emerald-400 font-bold uppercase block">Current Team Submission:</span>
                    <p className="font-bold text-white">{hackathon.userTeam.submission.projectName}</p>
                    <a
                      href={hackathon.userTeam.submission.repoUrl}
                      target="_blank"
                      rel="noreferrer"
                    ...
                    >
                      <FileCode2 className="w-3.5 h-3.5" />
                      <span>{hackathon.userTeam.submission.repoUrl}</span>
                    </a>
                  </div>
                )}
              </div>
                  ...
                  {hackathon.registrationMode !== "TEAM_ONLY" && (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold text-blue-400">Solo</span>
                      </div>
              ...
              <form onSubmit={handleCreateTeam} className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">
                    You will be assigned as Team Leader and receive a Join Code to invite up to {hackathon.maxTeamSize || 4} members.
                  </p>
                </div>
